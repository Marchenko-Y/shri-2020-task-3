...
Проект не собирался, по этому я починила TS ошибки

*1)* Файл server.ts  строка 82:
```
const validateProperty = (
   ...
   /^[A-Z]+$/.test(property.key.value)
       ...
       : [];
```
Здесь наверное опечатка, тип jsonToAst.AstProperty не содержит данных в property.key.loc но содержит в property.loc.

*2)* Файл server.ts строка 23
```
...
textDocumentSync имеет не то значение, что ожидалось. Может принять только значения TextDocumentSyncOptions 0 | 1 | 2 | undefined . Я выбрала вариант 1, так как он подходит ближе всего к always по комментарию


### Варнинги линтера
в проекте добавила новую команду linting
```
"linting": "tslint -c tslint.json 'src/**/*.ts'"
```
Запустила команду и исправила все варнинги

### Очистила от лишнего кода
В проекте были не используемые импорты и я их удалила
